...
def mergingStep2(clusters, MECs, merging_ratio):
    ...
    for cluster in clusters:
        if notIn(cluster[0], new_clusters):
            new_clusters.append(cluster)
    print("using m_r = ", merging_ratio, ", ", len(clusters), "->", len(new_clusters))
    return new_clusters
...
def DT_Clustering(points, sl_ratio, merging_ratio, spacingLevel, n):
    tri = Delaunay(points)
    clusters = []
    areas = []
    maxSideLengths = []
    ...
    merge_counter = 0
    merge_increment = 0.05
    reduction_factor = 0.9
    while (len(clusters) > n):
        merge_counter += 1
        new_clusters = mergingStep2(clusters, MECs, merging_ratio)
        if (len(new_clusters) < n):
            merge_increment = merge_increment*reduction_factor
            merging_ratio = merging_ratio-merge_increment
            if (merging_ratio < 0.0 or merge_increment<0.01):
                displayMEC(clusters, MECs, f'Post Merging{merge_counter}')
                break
        if (len(new_clusters) == len(clusters)):
            merging_ratio = merging_ratio+merge_increment
        elif (len(new_clusters) >= n):
...
sl_ratio = 0.3
merging_ratio = 0.9
spacingLevel = 12 # higher spacingLevel => more points clustered together
n = 3
plotMerges = 0
#####################################

result_data = DT_Clustering(points[:], sl_ratio,
                            merging_ratio, spacingLevel, n)
